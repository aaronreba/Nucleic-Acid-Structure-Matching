...
def main(argv=None):
    ...
    while 1:
        ...
        while 1:
            ...
            for value in bindParsedDataLine:
                if type(value) == type(''):
                    ...
                    preceedingBind = False
                    if copyBinds[bindIndex] > 0:
                        nextPossibleDataLine.append(copyBinds[bindIndex])
                    bindIndex += 1
                elif proceedingBind:
                    proceedingBind = False
                    appendMe = copyBinds[bindIndex - proceedingBinds]
                    proceedingBinds += 1
                    if appendMe > 0:
                        nextPossibleDataLine.append(appendMe)
            ...
            if copyNumbers == [0] * len(copyNumbers) and\
            copyBinds == [0] * len(copyBinds):
                break
            ...
            for reversedIndex in xrange(len(copyBinds) - 1, -1, -1):
                if copyBinds[reversedIndex] == 0:
                    copyBinds[reversedIndex] = allBinds[reversedIndex]
                else:
                    copyBinds[reversedIndex] -= 1
                    break
        ...
        for possibleDataLine in possibleDataLines:
                ...
                if type(value) == type(''):
                    ...
                    if preceedingBind:
                        ...
                        if lastBind == 'close':
                            ...
                            for i in xrange(-value, 0):
                                closedBindingLine.insert(0, addIndex + i)
                        else:
                            #insert at the end.
            ...
            #using xrange
            #go through the incremental line and bind lines to create all
            #permutations.
            k = 0
            
            while k < 4 ** countIncrements:
                #print incrementalLine
                ...
                nextIncrement = True
                for openIndex in openBindingLine:
                    value = incrementalLine[openIndex]
                    
                    if nextIncrement:
                        incrementalLine[openIndex] += 1
                        nextIncrement = False
                        if incrementalLine[openIndex] == 4:
                            incrementalLine[openIndex] = 0
                            nextIncrement = True
